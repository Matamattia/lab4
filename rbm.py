...
class RestrictedBoltzmannMachine():
          ...
          for i in range(epochs):
             ...
             # [TODO TASK 4.1] run k=1 alternating Gibbs sampling : v_0 -> h_0 ->  v_1 -> h_1.
                 ...
                 if (it+1) % self.rf["period"] == 0 and self.is_bottom:
                     ...
                     viz_rf(weights=self.weight_vh[:,self.rf["ids"]].reshape((self.image_size[0],self.image_size[1],-1)), it=it+1, grid=self.rf["grid"])
                 # print progress
      ...
      def update_params(self,v_0,h_0,v_k,h_k):
          ...
          self.delta_weight_vh = self.learning_rate*(np.dot(v_0.T, h_0) - np.dot(v_k.T, h_k)) / self.batch_size+self.momentum*(self.delta_weight_vh)
      ...
      def get_v_given_h(self,hidden_minibatch):
          ...
          if self.is_top:
              ...
              data=support[:, :-self.n_labels]
              labels=support[:, -self.n_labels:]
              prob_data=sigmoid(data)
              prob_labels=softmax(labels)
              v = np.concatenate((sample_binary(prob_data),sample_categorical(prob_labels)),axis=1)
              prob_v=np.concatenate((prob_data,prob_labels),axis=1)
